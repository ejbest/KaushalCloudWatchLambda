...
## Remove old files
script=ej_lambda.py
[ ! -f ${script}  ] && echo "Failed to locate file with function:  ${script}!" && exit  -1
...
[ ! -z $myarn ] && ( myid=`echo $myarn|cut -d':' -f5`; echo " ...OK: [myid=${myid}; myarn=${myarn}]") || (echo '  ... FAILED Creating role!';exit -1)
